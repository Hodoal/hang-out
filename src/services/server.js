...
// Conexión a MongoDB
mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/travelMoodApp')
...
app.get('/api/recommendations', verifyToken, async (req, res) => {
  try {
    ...
    if (!mood) {
      return res.status(400).json({ message: 'Se requiere especificar un estado de ánimo' });
    }
    ...
    const recommendedPlaces = generateRecommendations(mood, preferences.placeTypes);
...
function generateRecommendations(mood, placeTypes) {
  ...
  const placesByType = {
    restaurants: [
      { id: 'r1', name: 'Restaurante Gourmet', rating: 4.5, type: 'restaurants', 
        recommendedFor: ['relaxed', 'romantic', 'foodie'] },
      { id: 'r2', name: 'Food Truck Park', rating: 4.2, type: 'restaurants', 
        recommendedFor: ['adventurous', 'party', 'foodie'] }
    ],
    museums: [
      { id: 'm1', name: 'Museo de Arte Contemporáneo', rating: 4.7, type: 'museums', 
        recommendedFor: ['cultural', 'relaxed'] },
      { id: 'm2', name: 'Museo Interactivo', rating: 4.3, type: 'museums', 
        recommendedFor: ['adventurous', 'cultural'] }
    ],
    parks: [
      { id: 'p1', name: 'Parque Central', rating: 4.4, type: 'parks', 
        recommendedFor: ['relaxed', 'romantic', 'nature'] },
      { id: 'p2', name: 'Parque de Aventuras', rating: 4.6, type: 'parks', 
        recommendedFor: ['adventurous', 'nature'] }
    ],
    bars: [
      { id: 'b1', name: 'Bar Lounge', rating: 4.2, type: 'bars', 
        recommendedFor: ['romantic', 'relaxed'] },
      { id: 'b2', name: 'Bar de Cócteles', rating: 4.5, type: 'bars', 
        recommendedFor: ['party', 'romantic'] }
    ],
    beaches: [
      { id: 'be1', name: 'Playa Tranquila', rating: 4.8, type: 'beaches', 
        recommendedFor: ['relaxed', 'romantic', 'nature'] },
      { id: 'be2', name: 'Playa para Surf', rating: 4.7, type: 'beaches', 
        recommendedFor: ['adventurous', 'party', 'nature'] }
    ],
    historical: [
      { id: 'h1', name: 'Centro Histórico', rating: 4.6, type: 'historical', 
        recommendedFor: ['cultural', 'relaxed'] },
      { id: 'h2', name: 'Ruinas Antiguas', rating: 4.5, type: 'historical', 
        recommendedFor: ['cultural', 'adventurous'] }
    ],
    shopping: [
      { id: 's1', name: 'Centro Comercial', rating: 4.0, type: 'shopping', 
        recommendedFor: ['shopping', 'relaxed'] },
      { id: 's2', name: 'Mercado Local', rating: 4.4, type: 'shopping', 
        recommendedFor: ['shopping', 'cultural', 'foodie'] }
    ],
    entertainment: [
      { id: 'e1', name: 'Teatro', rating: 4.3, type: 'entertainment', 
        recommendedFor: ['cultural', 'romantic'] },
      { id: 'e2', name: 'Club Nocturno', rating: 4.1, type: 'entertainment', 
